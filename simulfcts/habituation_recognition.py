...
def initialize_recognition(id, gp, odors_gp,
    ...
    # TODO: deal with different models
    snaps_dict = {
...
def initialize_integration_lambda(id, lambd, gp, attrs, params, modopt, back, inits, spseed):
    """ Create a list of args and dictionary of kwargs to launch
    the main habituation parallel runs.
    """
    # Select appropriate functions for the model and background
...
def main_habituation_runs_lambda(filename, attributes, parameters, model_options):
    ...
    if attributes["model"] == "IBCM":
        ...
        lambd_range = np.geomspace(0.1, 2.0, repeats[0]) * lambda_0
    ...
    def callback(result):
        ...
        save_simul_results_lambda(sim_id, sim_results, attributes, sim_gp, snap_idx)
        print("Habituation run {} saved".format(sim_id))
        del sim_results, result
        return sim_id
