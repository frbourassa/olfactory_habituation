...
def generate_odor_tanhcdf(n_rec, rgen, alpha=0.38751946, 
        logb=-1.29460373, unit_scale=5e-4, krange=5e2):
    """ Generate vectors of active complex affinities K^* for an odor, 
    following a fit on the data of Si et al., 2019.  These affinities can be 
    up to 10^9 in Si et al.'s units, but here we prevent extreme values by 
    clipping them to k2range times the lower cutoff sensitivity in Si et al., 
    which is at roughly (1/b)^{1/alpha} * unit_scale. 
    ...
    Args:
        ...
        krange (tuple of 2 floats): lowest and highest 1/K* possible
        alpha (float): power law has CDF with exponent alpha, 
            PDF with exponent alpha-1
        unit_scale (float): constant factor to adjust the units of the
            affinities to our concentration scale. 
    ...
    kvec = inverse_transform_tanhcdf(r, logb, alpha) * unit_scale
    # In the sampled distribution, the lower cutoff is at x^alpha=1/b, roughly
    kvec = np.clip(kvec, 0.0, krange * unit_scale * 10.0**(-logb/alpha))
    return kvec


def combine_odors_affinities(concs, kmat, epsils, fmax=1.0):
    """ Simplified version of the Kadakia and Emonet model where we neglect
    the inactive complex affinities K so odors are defined by their affinity
    for active complexes, K^*, and assume K^* c >> 1 to get a simplified form
    where the baseline remains zero as we tune the level of nonlinear saturation
    via the free energy differences, epsilon. 

    Args:
        concs (np.ndarray): 1d array of odor concentrations, indexed [n_odors]
        kmat (np.ndarray): shape [n_odors, n_osns], 
            affinities of active complexes (neglecting inactive ones)
...
def sample_background_powerlaw_nl_osn(kmat, *args, size=1, rgen=None):
    """
    Args:
        kmat (np.ndarray): array of background odor affinities K^*,
    ...
    vec_samp = combine_odors_affinities(nu_samp, kmat, epsils, fmax=fmax_osn)
    return vec_samp, nu_samp


# Original version that does not neglect K or 1 in 1 + K*c
# It does not significantly change the results since K is small anyways, 
# but it makes controlling the nonlinearity more complicated, 
# so we use the simplified version in full simulations
def generate_odor_tanhcdf_full(n_rec, rgen, k1val=0.01, alpha=0.38751946, 
...
def combine_odors_affinities_original(concs, kmats, epsils, fmax=1.0):
...
def combine_odors_affinities_original_0baseline(concs, kmats, epsils, fmax=1.0):
    """ Combine odors coming in with concentrations conc and defined
    by active and inactive binding affinities kappa1, kappa2. 
