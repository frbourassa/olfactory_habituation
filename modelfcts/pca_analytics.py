...
def fixedpoints_pca_2vectors(components, sigma2, ab_rates, lambd=1.0):
    """
    ...
    m_pca = sigma2 * l2_norm(x_s) * x_s * lambd
    w_pca = m_pca / (l_pca*lambd**2 + ba_ratio)
    return m_pca, l_pca, w_pca


def pca_fixedpoint_s_2vectors_instant(components, sigma2, ab_rates, xser, lambd=1.0):
    ba_ratio = ab_rates[1] / ab_rates[0]
    x_d = np.sum(components, axis=0)/2.0
    x_s = np.diff(components, axis=0)

    return ba_ratio / (ba_ratio + sigma2*l2_norm(x_s)*lambd**2) * (xser - x_d)

def pca_fixedpoint_s_2vectors_variance(components, sigma2, ab_rates, lambd=1.0):
    ba_ratio = ab_rates[1] / ab_rates[0]
    x_d = np.sum(components, axis=0)/2.0
    x_s = np.diff(components, axis=0)
    pc = sigma2 * l2_norm(x_s) * lambd**2
