...
### Experimental stuff
# Model with feedback on LNs to orient W with a component along the expected average new odor

# Feedback function: to add to s vector as input to LNs
def feedback(yvec, params):
    expect_c_elem, target_ynorm = params
    # Positive difference, add input if ynorm smaller than target
    diff = np.tanh(np.maximum(0.0, target_ynorm - l2_norm(yvec, axis=-1)) * 4.0)
    return diff * expect_c_elem

### NETWORK OF IBCM NEURONS WITH OPTIMAL INHIBITORY WEIGHTS
# One main function instead of multiple variants
# Only marginal performance sacrifices by checking the various options.
def integrate_inhib_ibcm_feedback(vari_inits, update_bk, bk_init,
    ibcm_params, inhib_params, bk_params, tmax, dt, feedback_params,
    seed=None, noisetype="normal", skp=1, **options):
    r""" Integrate the IBCM equation when the input updated by the derivative
    function update_bk, which takes pre-generated noise and bk_params.
    ...
    Args:
        ...
        tmax (float): max time
        dt (float): time step
        feedback_params (list of 2 floats): expected new odor concentration, 
            and target y norm
    ...
    bk_vari = bk_vari_init.copy()
    bkvec = bk_vec_init.copy()
    c = m.dot(bkvec + feedback(bkvec, feedback_params))  # un-inhibited neuron activities
    # Initialize neuron activity with m and background at time zero
    cbar = c - coupling*(np.sum(c) - c)  # -c to cancel the subtraction of c[i] itself
    if saturation == "tanh":
        sat_abs = sat * lambd
    ...
    for k in range(0, len(tseries)*skp-1):
        ...
        # with the updated background and synaptic weight vector m
        # Compute un-inhibited activity of each neuron with current input (at time k)
        # Unsure about feedback: should it use step k's svec with step k+1's bkvec?
        # Doesn't seem to have a choice, can't update otherwise. Assume
        # there is a small delay in how the feedback is applied: good for new
        # odor recognition, first step with new odor gets feedback from
        # last step with background only. 
        c = m.dot(bkvec + feedback(svec, feedback_params))
        cbar = c - coupling*(np.sum(c) - c)  # -c to cancel the subtraction of c[i] itself
        if saturation == "tanh":
            cbar = sat_abs * np.tanh(cbar / sat_abs)
        # np.sum(c) is a scalar and c a vector, so it broadcasts properly.
...
def ibcm_respond_new_odors_feedback(
        odors, mmat, wmat, ibcm_rates, feedback_params, options={}
    ):
    """
    Args:
        odors (np.ndarray): indexed [..., n_orn]
        ...
        wmat (np.ndarray): indexed [n_orn, n_neurons]
        ibcm_rates (np.ndarray): contains coupling eta and tanh amplitude
        feedback_params (list of floats): new odor conc., 
            threshold below at which feedback is applied
        options (dict):
            activ_fct (str): either "ReLU" or "identity"
            saturation (str): "tanh" or "linear"
    """
    ...
    # now do this again, but now with the feedback
    c = (odors + feedback(svec, feedback_params).reshape(-1, 1)).dot(mmat.T)
    cbar = c - eta*(np.sum(c, axis=-1, keepdims=True) - c)
    if saturation == "tanh":
        cbar = sat * np.tanh(cbar / sat)
    svec = odors - cbar.dot(wmat.T)
