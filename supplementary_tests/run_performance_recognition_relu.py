...
if __name__ == "__main__":
    ...
    dummy_rgen = np.random.default_rng(0x5a934e97ac60044f211cf021ea11ff37)
    ...
    ### IBCM RUNS ###
    ibcm_file_name = os.path.join(folder, "ibcm_performance_results_relu.h5")
    ibcm_attrs = {
        "model": "IBCM",
    ...
    n_i = n_b
    dimensions_array = np.asarray([n_r, n_b, n_i, n_k])
    pca_file_name = os.path.join(folder, "biopca_performance_results_relu.h5")
    biopca_attrs = {
        "model": "PCA",
        "background": "turbulent",
    ...
    n_i = 1
    dimensions_array = np.asarray([n_r, n_b, n_i, n_k])
    avg_file_name = os.path.join(folder, "avgsub_performance_results_relu.h5")
    avg_attrs = {
        "model": "AVG",
        "background": "turbulent",
        # Intentionally the same seed to test all models against same backs
    ...
    for kind in ["orthogonal", "ideal", "optimal", "none"]:
        ...
        ideal_file_name = os.path.join(folder, 
                    kind+"_performance_results_relu.h5")
        idealized_recognition_from_runs(ideal_file_name, 
                    ibcm_file_name, kind, lean=True)
